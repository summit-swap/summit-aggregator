...
class AdapterTestEnv {
  ...
  async queryMatches(amountIn, tokenFromAdd, tokenToAdd, expectedAmountOut) {
    const amountOutQuery = await this.query(amountIn, tokenFromAdd, tokenToAdd);
    expect(amountOutQuery).to.eq(expectedAmountOut);
  }

  async checkSwapMatchesQuery(dxFixed, tokenFrom, tokenTo) {
    await this.checkSwapMatchesQueryWithDustWithErr(dxFixed, tokenFrom, tokenTo, BN_ZERO, DEFAULT_ERROR_BPS);
  }

  async checkSwapMatchesQueryWithErr(dxFixed, tokenFrom, tokenTo, errorBps) {
    await this.checkSwapMatchesQueryWithDustWithErr(dxFixed, tokenFrom, tokenTo, BN_ZERO, errorBps);
  }

  async checkSwapMatchesQueryWithDust(dxFixed, tokenFrom, tokenTo, maxDustWei) {
    await this.checkSwapMatchesQueryWithDustWithErr(dxFixed, tokenFrom, tokenTo, maxDustWei, DEFAULT_ERROR_BPS);
  }

  async checkSwapMatchesQueryWithDustWithErr(dxFixed, tokenFrom, tokenTo, maxDustWei, errorBps) {
    const { swapFn, queryDy } = await this.#getQueryDyAndSwapFn(dxFixed, tokenFrom, tokenTo);
    expect(queryDy).gt(0);
    const balDiffs = await this.#executeAndReturnBalChange(swapFn, tokenTo, [
      this.trader().address,
  ...
  async checkGasEstimateIsSensible(options, accuracyPct = 10) {
    ...
    for (let [amountInFixed, tokenFrom, tokenTo] of options) {
      ...
      const gasUsed = await this.#getGasEstimateForSwapAndQuery(amountIn, tokenFrom, tokenTo);
      if (gasUsed > maxGas) {
        maxGas = gasUsed;
      }
    }
  ...
  async #getGasEstimateForQuery(amountIn, tokenFrom, tokenTo) {
    return this.Adapter.estimateGas.query(amountIn, tokenFrom.address, tokenTo.address).then(parseInt);
  }

  async #getGasEstimateForSwap(amountIn, tokenFrom, tokenTo) {
    const minDy = parseUnits("1", "wei");
    const txReceipt = await this.mintAndSwap(amountIn, minDy, tokenFrom, tokenTo).then((tx) => tx.wait());
    return parseInt(txReceipt.gasUsed);
  }

  async #getQueryDyAndSwapFn(dxFixed, tokenFrom, tokenTo) {
    const amountIn = await parseUnitsForTkn(dxFixed, tokenFrom);
    const queryDy = await this.query(amountIn, tokenFrom.address, tokenTo.address);
    const swapFn = async () => this.mintAndSwap(amountIn, queryDy, tokenFrom, tokenTo);
    return { queryDy, swapFn };
  }

  async query(amountIn, tokenFromAdd, tokenToAdd) {
    return this.Adapter.query(amountIn, tokenFromAdd, tokenToAdd);
  }

  async mintAndSwap(amountIn, amountOutQuery, tokenFrom, tokenTo, to) {
  ...
  async #swap(amountIn, amountOutQuery, tokenFrom, tokenTo, to = this.trader().address) {
    return this.Adapter.connect(this.trader()).swap(amountIn, amountOutQuery, tokenFrom.address, tokenTo.address, to);
  }
}
