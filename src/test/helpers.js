...
module.exports.getAccountsGen = async () => {
  ...
  function* makeGen() {
    for (let account of accounts) yield account;
  }
...
module.exports.getERC20Decimals = (provider, token) => {
  return provider.call({ to: token, data: "0x313ce567" }).then((d) => parseInt(d, 16));
};
...
const _getTokenContract = (tokenAddress) =>
  ethers.getContractAt("src/contracts/interface/IERC20.sol:IERC20", tokenAddress);
...
module.exports.setERC20Bal = async (_token, _holder, _amount) => {
  ...
  const index = ethers.utils.keccak256(key + storageSlot.toString(16).padStart(64, "0")).replace(/0x0+/, "0x"); // Hardhat doesn't like leading zeroes
...
module.exports.injectFunds = async (sender, reciever, amount) => {
  await ethers.getContractFactory("NTInjector").then((f) => f.connect(sender).deploy(reciever, { value: amount }));
};
...
module.exports.forkGlobalNetwork = async (_blockNumber, _networkId) => {
  ...
  if (!networkConfig) throw new Error(`Network-ID "${_networkId}" not recognized`);
  _setHardhatNetwork({
