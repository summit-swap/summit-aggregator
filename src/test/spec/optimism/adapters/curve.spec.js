...
describe("SummitAdapter - Curve", () => {
  ...
  describe("wsteth", () => {
    before(async () => {
      ...
      const adapterArgs = ["CurveWstethAdapter", 250_000, curve.plain128_native.wsteth_eth, tkns.WETH.address];
      ate = await testEnv.setAdapterEnv(contractName, adapterArgs);
    });

    describe("Swapping matches query", async () => {
      it("20 ETH -> wstETH", async () => {
        await ate.checkSwapMatchesQueryWithDust("20", tkns.WETH, tkns.wstETH, MaxDustWei);
      });
      it("134 wstETH -> ETH", async () => {
        await ate.checkSwapMatchesQueryWithDust("134", tkns.wstETH, tkns.WETH, MaxDustWei);
      });
    });
