...
describe("SummitAdapter - Fusion", function () {
  let testEnv;
  let tkns;
  ...
  before(async () => {
    ...
    const adapterArgs = ["FusionAdapter", gasEstimate, quoterGasLimit, fusion.quoter, fusion.factory, defaultFees];
  ...
  it("Swapping too much returns zero", async () => {
    const dy = await ate.Adapter.query(ethers.utils.parseUnits("10000", 18), tkns.USDC.address, tkns.WMNT.address);
    expect(dy).to.eq(0);
  });
