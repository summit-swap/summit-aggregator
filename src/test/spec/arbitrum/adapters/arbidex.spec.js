...
describe("SummitAdapter - ArbiDex", function () {
  let testEnv;
  let tkns;
  ...
  before(async () => {
    ...
    const adapterArgs = ["ArbiDexAdapter", gasEstimate, quoterGasLimit, arbidex.quoter, arbidex.factory, defaultFees];
  ...
  it("Most liquid pool offers best quote", async () => {
    ...
    for (let fee of feeOptions) {
      ...
      if (quote.gt(bestQuote)) bestQuote = quote;
    }
  ...
  it("Swapping too much returns zero", async () => {
    const dy = await ate.Adapter.query(ethers.utils.parseUnits("10000", 18), tkns.WETH.address, tkns.WBTC.address);
    expect(dy).to.eq(0);
  });
