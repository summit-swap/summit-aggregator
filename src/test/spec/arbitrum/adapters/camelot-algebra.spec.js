...
describe("SummitAdapter - CamelotAlgebra", function () {
  let testEnv;
  let tkns;
  ...
  before(async () => {
    ...
    const adapterArgs = ["CamelotAlgebra", gasEstimate, quoterGasLimit, other.algebraQuoter, camelot.algebraFactory];
  ...
  it("Swapping too much returns zero", async () => {
    const dy = await ate.Adapter.query(ethers.utils.parseUnits("1000000000", 18), tkns.WETH.address, tkns.ARB.address);
    expect(dy).to.eq(0);
  });
