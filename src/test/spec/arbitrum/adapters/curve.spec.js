...
describe("SummitAdapter - Curve", function () {
  ...
  describe("TwoStable", async () => {
    ...
    describe("Swapping matches query", async () => {
      it("100 USDT -> USDC", async () => {
        await ate.checkSwapMatchesQueryWithDust("100", tkns.USDT, tkns.USDC, MaxAdapterDust);
      });
      it("10000 USDT -> USDC", async () => {
        await ate.checkSwapMatchesQueryWithDust("10000", tkns.USDT, tkns.USDC, MaxAdapterDust);
      });
      it("100 USDC -> USDT", async () => {
        await ate.checkSwapMatchesQueryWithDust("100", tkns.USDC, tkns.USDT, MaxAdapterDust);
      });
    });
  ...
  describe("TriCrypto", async () => {
    ...
    describe("Swapping matches query", async () => {
      it("2000000 USDT -> WETH", async () => {
        await ate.checkSwapMatchesQueryWithDust("2000000", tkns.USDT, tkns.WETH, MaxAdapterDust);
      });
      it("1000 WETH -> WBTC", async () => {
        await ate.checkSwapMatchesQueryWithDust("1000", tkns.WETH, tkns.WBTC, MaxAdapterDust);
      });
      it("300 WBTC -> USDT", async () => {
        await ate.checkSwapMatchesQueryWithDust("300", tkns.WBTC, tkns.USDT, MaxAdapterDust);
      });
    });
  ...
  describe("meta", async () => {
    ...
    describe("Swapping matches query", async () => {
      it("20000 FRAX -> USDT", async () => {
        await ate.checkSwapMatchesQueryWithErr("20000", tkns.FRAX, tkns.USDT, MaxErrorBps);
      });
      it("1 USDC -> FRAX", async () => {
        await ate.checkSwapMatchesQueryWithErr("1", tkns.USDC, tkns.FRAX, MaxErrorBps);
      });
      it("2222 USDC -> MIM", async () => {
        await ate.checkSwapMatchesQueryWithErr("2222", tkns.USDC, tkns.MIM, MaxErrorBps);
      });

      it("333331 MIM -> USDT", async () => {
        await ate.checkSwapMatchesQueryWithErr("333331", tkns.MIM, tkns.USDT, MaxErrorBps);
      });
    });

    it("Query returns zero if trade is between two underlying tokens", async () => {
      expect(await ate.query(parseUnits("1", 6), tkns.USDC.address, tkns.USDT.address)).to.eq(ethers.constants.Zero);
    });
