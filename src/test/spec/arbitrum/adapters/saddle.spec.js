  ...
  describe("meta_arbusdV2", () => {
    ...
    describe("Swapping matches query with error", async () => {
      it("30 USDs -> USDC", async () => {
        await ate.checkSwapMatchesQueryWithErr("30", tkns.USDs, tkns.USDC, maxErrorBps);
      });
      it("1000000 USDs -> USDT", async () => {
        await ate.checkSwapMatchesQueryWithErr("100000", tkns.USDs, tkns.USDT, maxErrorBps);
      });
      it("1 USDs -> FRAX", async () => {
        await ate.checkSwapMatchesQueryWithErr("1", tkns.USDs, tkns.FRAX, maxErrorBps);
      });
      it("2133 USDC -> USDs", async () => {
        await ate.checkSwapMatchesQueryWithErr("2133", tkns.USDC, tkns.USDs, maxErrorBps);
      });
      it("2133 USDT -> USDs", async () => {
        await ate.checkSwapMatchesQueryWithErr("2133", tkns.USDT, tkns.USDs, maxErrorBps);
      });
      it("2133 FRAX -> USDs", async () => {
        await ate.checkSwapMatchesQueryWithErr("2133", tkns.FRAX, tkns.USDs, maxErrorBps);
      });
    });
