...
const { setTestEnv, addresses, constants, helpers } = require("../../../utils/test-env");
const { assets, other } = addresses.avalanche;
const { parseUnits, formatUnits } = ethers.utils;
const { yyPlanet } = constants.geode;

describe("SummitAdapter - geode", () => {
  let testEnv;
  let tkns;
  ...
  before(async () => {
    ...
    const adapterArgs = ["GWPyyAvaxAdapter", other.GeodePortal, constants.geode.yyPlanet, 346_000];
  ...
  describe("maintanance", async () => {
    ...
    it("set valid interface", async () => {
      ...
      await expect(Adapter.setInterfaceForPooledTkn(validInterface)).to.not.reverted;
    });
    ...
    it("revoke/set token allowance", async () => {
      ...
      const approved = await gAVAX.isApprovedForAll(Adapter.address, GeodeWP.address);
      if (!approved) {
      ...
      expect(await gAVAX.isApprovedForAll(Adapter.address, GeodeWP.address)).to.be.false;

      // Set allowance and check allowance is set
      await Adapter.setGAvaxAllowance();
      expect(await gAVAX.isApprovedForAll(Adapter.address, GeodeWP.address)).to.be.true;
    });
  });
  ...
  describe("Swapping matches query", async () => {
    ...
    async function createDebt() {
      ...
      const pooledAvax = await ethers.provider.getBalance(GeodeWP.address);
      const avaxIn = pooledAvax.div(2);
      // Mint yyavax
      const deadline = Math.floor(Date.now() / 1e3) + 300;
      await Portal.connect(trader).stake(ethers.BigNumber.from(yyPlanet), avaxIn.div(2), deadline, { value: avaxIn });
      const yyAvaxIn = await tkns.yyAVAX.balanceOf(trader.address);
      await tkns.yyAVAX.connect(trader).transfer(Adapter.address, yyAvaxIn);
      // Sell yyavax
      await Adapter.swap(yyAvaxIn, 0, assets.yyAVAX, assets.WAVAX, trader.address);
    ...
    async function removeDebt() {
      ...
      if (debt.gt(minDebt)) {
        const deadline = Math.floor(Date.now() / 1e3) + 300;
        await Portal.connect(trader).stake(ethers.BigNumber.from(yyPlanet), 0, deadline, { value: debt });
    ...
    describe("staking is not paused", async () => {
      ...
      describe("wavax -> yyavax", async () => {
        ...
        describe("debt < amountIn", async () => {
          ...
          async function checkAdapterQueryGtThanOriginal(tokenFrom, tokenTo, amountIn) {
            const [ti0, ti1] = tokenFrom.address == assets.WAVAX ? [0, 1] : [1, 0];
            // Query original contract
            const amountOutOriginal = await GeodeWP.calculateSwap(ti0, ti1, amountIn);
            // Query adapter
            const amountOutAdapter = await Adapter.query(amountIn, tokenFrom.address, tokenTo.address);
            // Compare two prices
          ...
          it("query should offer better price than original", async () => {
            ...
            await checkAdapterQueryGtThanOriginal(tknIn, tknOut, amountIn);
          });
      ...
      describe("yyavax->wavax", async () => {
        ...
        it("swap should match the query", async () => {
          ...
          await Adapter.connect(trader).swap(amountIn.mul(2), 0, tknOut.address, tknIn.address, Adapter.address);
          // Check
          await ate.checkSwapMatchesQuery(formatUnits(amountIn, 18), tknIn, tknOut);
        });
      });
    });
  });
  ...
  it("Gas-estimate is between max-gas-used and 110% max-gas-used", async () => {
    const options = [["1", tkns.WAVAX, tkns.yyAVAX]];
