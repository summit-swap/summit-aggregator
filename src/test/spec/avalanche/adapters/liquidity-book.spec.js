...
describe("SummitAdapter - LiquidityBook", function () {
  let testEnv;
  let tkns;
  ...
  beforeEach(async () => {
    ...
    const adapterArgs = ["LiquidityBookAdapter", gasEstimate, liquidityBook.router];
  ...
  it("Swapping too much returns zero", async () => {
    const dy = await ate.Adapter.query(ethers.utils.parseUnits("100000000", 6), tkns.USDC.address, tkns.USDT.address);
    expect(dy).to.eq(0);
  });
