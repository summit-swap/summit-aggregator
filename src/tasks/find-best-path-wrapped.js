...
task("find-best-path-wrapped", "Queries SummitWrapRouter for the best path", async (args, hre) => {
...
class Quoter {
  ...
  async getBestOffer(swapArgs) {
    const SummitRouter = await this.hre.ethers.getContract("SummitWrapRouter")
    console.log(swapArgs.wrapper);
    const wrapper = await this.hre.ethers.getContractAt("src/contracts/SummitWrapper.sol:SummitWrapper", swapArgs.wrapper)
      const offer = (swapArgs.tokenIn == await wrapper.getWrappedToken()) ? await SummitRouter.unwrapAndFindBestPath(
        swapArgs.amountIn, 
        swapArgs.tokenOut, 
        swapArgs.wrapper, 
        swapArgs.maxHops
      ) :
      await SummitRouter.findBestPathAndWrap(
        swapArgs.amountIn, 
        swapArgs.tokenIn, 
        swapArgs.wrapper, 
        swapArgs.maxHops
