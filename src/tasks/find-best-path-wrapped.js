...
class Quoter {
  ...
  parseAddress(addressAlias) {
    ...
    if (address == undefined) throw new Error(`Address-alias ${addressAlias} not recognized`);
    return address;
  }
  ...
  async getBestOffer(swapArgs) {
    ...
    const wrapper = await this.hre.ethers.getContractAt(
      "src/contracts/SummitWrapper.sol:SummitWrapper",
      swapArgs.wrapper
    );
    const offer =
      swapArgs.tokenIn == (await wrapper.getWrappedToken())
        ? await SummitRouter.unwrapAndFindBestPath(
            swapArgs.amountIn,
            swapArgs.tokenOut,
            swapArgs.wrapper,
            swapArgs.maxHops
