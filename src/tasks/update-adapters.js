...
task(
  ...
  async function (_, hre, _) {
    ...
    await updateAdapters(SummitRouter, deployer, adaptersWhitelist);
  }
);
...
async function updateAdapters(summitRouter, deployerSigner, adaptersWhitelist) {
  let currentAdapters = await getAdaptersForRouter(summitRouter);
  ...
  if (prompt("Proceed to set adapters? y/n") == "y") {
    await summitRouter.connect(deployerSigner).setAdapters(adaptersWhitelist).then(finale);
  }
}
...
async function getAdapterWhitelist(deployments, networkId) {
  ...
  return Promise.all(
    whitelistNamed.map((adapterName) => {
...
async function getAdaptersForRouter(summitRouter) {
  let adapterCount = await summitRouter.adaptersCount().then((r) => r.toNumber());
  return Promise.all([...Array(adapterCount).keys()].map((i) => summitRouter.ADAPTERS(i)));
}
