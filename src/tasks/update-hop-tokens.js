...
task(
  "update-hop-tokens",
  "Updates SummitRouter's hop-tokens according to `deployOptions.json`.'",
  async function (_, hre, _) {
    const networkId = hre.network.name;
    const [deployer] = await hre.ethers.getSigners();
    const SummitRouter = await getRouterContract(networkId);
    const hopTokensWhitelist = await getHopTokensWhitelist(networkId);
    await updateHopTokens(SummitRouter, deployer, hopTokensWhitelist);
  }
);
...
function getRouterDeployment(networkId) {
  const path = `../deployments/${networkId}/SummitRouter.json`;
  try {
    return require(path);
  } catch {
...
async function getRouterContractForAddress(routerAddress) {
  return ethers.getContractAt("SummitRouter", routerAddress);
}

async function updateHopTokens(summitRouter, deployerSigner, hopTokensWhitelist) {
  const currentHopTokens = await getTrustedTokensForRouter(summitRouter);
  ...
  if (prompt("Proceed to set hop-tokens? y/n") == "y") {
    await summitRouter.connect(deployerSigner).setTrustedTokens(hopTokensWhitelist).then(finale);
  }
}
...
async function getTrustedTokensForRouter(summitRouter) {
  let trustedTokensCount = await summitRouter.trustedTokensCount().then((r) => r.toNumber());
  return Promise.all([...Array(trustedTokensCount).keys()].map((i) => summitRouter.TRUSTED_TOKENS(i)));
}
