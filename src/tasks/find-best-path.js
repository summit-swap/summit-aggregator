...
task("find-best-path", "Queries SummitRouter for the best path", async (args, hre) => {
...
class Quoter {
  ...
  async getBestOffer(swapArgs) {
    const SummitRouter = await this.hre.ethers.getContract("SummitRouter");
    const offer = await SummitRouter.findBestPath(
      swapArgs.amountIn,
      swapArgs.tokenIn,
      swapArgs.tokenOut,
      swapArgs.maxHops
