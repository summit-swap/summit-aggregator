...
import { SummitAdapterV2 } from "../SummitAdapterV2.sol";

contract UniswapV2AdapterV2 is SummitAdapterV2 {
    ...
    ) SummitAdapterV2(_name, _swapGasEstimate) {
    ...
    function _getTransferTarget(address _tokenIn, address _tokenOut) internal view override returns (address) {
        return IUniswapFactory(factory).getPair(_tokenIn, _tokenOut);
    }

    function _swap(uint256, uint256 _amountOut, address _tokenIn, address _tokenOut, address to) internal override {
        address pair = IUniswapFactory(factory).getPair(_tokenIn, _tokenOut);
